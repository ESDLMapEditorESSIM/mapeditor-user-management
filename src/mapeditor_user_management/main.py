...
def main():
    ...
    if args.mode == "add-users":
        ...
        if args.edit_users_from_csv:
            edit_usernames = [user.username for user in parse_users(args.edit_users_from_csv)]
            mongo_usernames = [user for user in edit_usernames if user in existing_users]

        # edit keycloak user settings
        if args.edit_mode in ["update", "overwrite"]:
            keycloak_edit_mode = "add"
        else:
            keycloak_edit_mode = "delete"
        keycloak_admin.edit_users_keycloak(
            keycloak_edit_mode,
            edit_usernames,
        ...
        if args.setting_name:
            ...
            for username in mongo_usernames:
                print(f"Setting user '{username}' config '{args.setting_name}' to file '{args.setting_value_file}'")
                if args.edit_mode == "update":
                    setting_value = mongo_interface.get_setting(
                        SettingType.USER, username, args.setting_name
                    )
